...
class Request {
  ...
  async httpFactory<T>({ url = '', params = {}, method }: Props): Promise<T> {
    const req = this.interceptorsRequest({
      url: url.startsWith('http') ? url : process.env.NEXT_PUBLIC_BASEURL + url,
      method,
  ...
  async request<T>(method: Method, url: string, params?: Params): Promise<T> {
    ...
    if (session && session.user) {
      const plain = session.user?.id + ":" + session.user?.name + ":" + session.user?.role
      const token = Buffer.from(plain, 'utf8').toString('base64')
      params = {
        ...params,
        headers: {
          ...params?.headers,
